...
class Main extends MY_Controller {
    ...
    public function contact() {
        ...
        $this->form_validation->set_rules('email','Ваш емаил', 'trim|required|valid_email');
        ...
        if ($this->form_validation->run() == FALSE ) {
            ...
            $thos->load->view('templates/footer');
        }
        else {
            ...
            $this->email->suject($subject);
            $this->email->message($message);
            if ($this->email->send()) {
                $this->session->set_flashdata('msg', '<div class="alert-success text-center">Ваше сообщение успешно
                    отправлено!</div>');
                redirect('/contact');
            }
            else {
                $this->session->set_flashdata('msg', '<div class="alert alert-danger text-center">Произошла ошибка, повторите
                    попытку позжею</div>');
                redirect('/contact');
            }
        }
