...
class Search extends MY_Controller {
    ...
    public function index() {
        ...
        if ($this->input->get('q_search')) {
            ...
            $this->data['search_result'] = $this->Search_model->($q, $row_count, $offset);

            //pagination
            $this->load->library('pagination');
            $p_config['suffix'] = '?' . http_build_query($_GET, '', "&");
            $count = count = count($this->Search_model->search($q, 0, 0));
            $p_config_['base_url'] = $p_config['base_url'].'?'.http_build_query($_GET);
